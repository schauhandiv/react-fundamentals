...
- I didn't get the exercise at all, I have no clue why is it working as expected, I just followed the instructions from Cody!!
- I didn't quite get the extra credit 1 either, just followed instructions 
- I started to enjoy by the time I started doing Extra credit 2... still not 100% what I am doing, but getting a hang of it and can't wait to watch the video and hear KCD's explanation
- On an onSubmit event (on a form), the Browser by default makes a GET request to the current URL with values of the form as query parameters (with tag name as the key and tag value as the value) to the url which causes a full page refresh. To prevent this default behaviour use `event.preventDefault()`
- _SyntheticEvent_ is an object that React creates for us that looks and behaves like a regular Browser event; it is not a real event from the Browser; this is for performance reasons. event.nativeEvent will show the native event (for onSubmit it is SubmitEvent object).
- `event.target` is a property on the SubmitEvent object. For the onSubmit event it is the entire form object. Chrome tools show some rendered output on the form DOM node on `console.log(event.target)`. To see the properties of that DOM node use `console.dir(event.target)`.
   - event.target[0].value is the first element in the form, and so forth
   - event.target.elements.usernameInput.value is to get the value by name/id of the element.
- Screen reader use Label associated with an input to read out loud. The Label's attribute 'htmlFor' holds the id of the input element. So, its preferebale to use id instead of name to identify an html element/tag. When the user clicks on the label the focus is set to the input associated with the label.
   `<label htmlFor="usernameInput">Username</label><input id="usenameInput" type="text" />`
- `event.target` is the input object for the onChange event on the input element.
...
Fill out
[the feedback form](https://ws.kcd.im/?ws=React%20Fundamentals%20%E2%9A%9B&e=06%3A%20Forms&em=schauhan%40divsionsinc.com).
